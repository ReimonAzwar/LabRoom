...
.time-sep{color:var(--text3);font-size:14px;text-align:center;padding-top:6px}
...
/* ─── ROOM CALENDAR POPUP ─── */
.rc-overlay{display:none;position:fixed;inset:0;background:rgba(13,34,68,.6);backdrop-filter:blur(6px);z-index:300;align-items:center;justify-content:center;padding:20px}
...
.rc-day{border-radius:7px;padding:5px 3px;text-align:center;cursor:pointer;transition:all .15s;min-height:44px;display:flex;flex-direction:column;align-items:center;gap:2px}
.rc-day:hover{background:var(--cream2)}
.rc-day.today{border:1.5px solid var(--navy);background:rgba(13,34,68,.04)}
.rc-day.has-bookings{background:rgba(200,168,75,.1)}
.rc-day.selected{background:var(--navy);border:1.5px solid var(--navy)}
.rc-day.selected .rc-day-num{color:var(--gold3)}
.rc-day.empty{cursor:default;pointer-events:none}
.rc-day-num{font-size:12px;font-weight:600;color:var(--text2)}
.rc-day.today .rc-day-num{color:var(--navy)}
.rc-day.has-bookings .rc-day-num{color:var(--gold2)}
...
/* ─── SCROLL ANIMATIONS ─── */
.anim-ready{opacity:0;transform:translateY(16px);transition:opacity .5s ease,transform .5s ease}
